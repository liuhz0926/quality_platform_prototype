...
* If there is a problem for FileNotFoundError: in view.py load_evaluate, the home_address now is set to '~/quality_platform_prototype', i.e. the app is in the user folder in MacOS. This needs to be reset if the app is uploaded to the cloud or changed to other folder. 
...
* If there is a problem for FileNotFoundError, but running the app still locally (i.e. not change the dataset_url to the cloud). please check the zip_directory, unzip_directory in Coco_request.py and self.test_file in post_predict() in Coco_request.py. 
